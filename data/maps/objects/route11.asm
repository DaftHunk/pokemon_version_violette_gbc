Route11Object:
	db $f ; border block

	db 5 ; warps
	warp 51, 8, 0, ROUTE_11_GATE_1F
	warp 51, 9, 1, ROUTE_11_GATE_1F
	warp 58, 8, 2, ROUTE_11_GATE_1F
	warp 58, 9, 3, ROUTE_11_GATE_1F
	warp 4, 5, 0, DIGLETTS_CAVE_ENTRANCE

	db 1 ; signs
	sign 1, 5, 11 ; Route11Text11

	db 10 ; objects
	object SPRITE_GAMBLER,          10, 14, STAY, DOWN,  1, OPP_GAMBLER,   1
	object SPRITE_GAMBLER,          26,  9, STAY, DOWN,  2, OPP_GAMBLER,   2
	object SPRITE_BLACK_HAIR_BOY_2, 13,  5, STAY, LEFT,  3, OPP_ENGINEER,  3
	object SPRITE_BLACK_HAIR_BOY_2, 36, 11, STAY, DOWN,  4, OPP_ENGINEER,  1
	object SPRITE_BLACK_HAIR_BOY_2, 22,  4, STAY, UP,    5, OPP_ENGINEER,  4
	object SPRITE_GAMBLER,          45,  7, STAY, DOWN,  6, OPP_GAMBLER,   3
	object SPRITE_GAMBLER,          33,  3, STAY, UP,    7, OPP_GAMBLER,   4
	object SPRITE_BUG_CATCHER,      43,  5, STAY, RIGHT, 8, OPP_YOUNGSTER, 8
	object SPRITE_BLACK_HAIR_BOY_2, 45, 16, STAY, LEFT,  9, OPP_ENGINEER,  2
	object SPRITE_BUG_CATCHER,      22, 12, STAY, UP,   10, OPP_YOUNGSTER, 9

	; warp-to
	warp_to 51, 8, ROUTE_11_WIDTH ; ROUTE_11_GATE_1F
	warp_to 51, 9, ROUTE_11_WIDTH ; ROUTE_11_GATE_1F
	warp_to 58, 8, ROUTE_11_WIDTH ; ROUTE_11_GATE_1F
	warp_to 58, 9, ROUTE_11_WIDTH ; ROUTE_11_GATE_1F
	warp_to 4, 5, ROUTE_11_WIDTH ; DIGLETTS_CAVE_ENTRANCE
