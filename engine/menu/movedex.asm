...
.printMoveLoop
	...
	CheckEvent EVENT_MOVEDEX_SORTING_MODE
...
.selectPressed
	ld a, SFX_SWITCH
	Call PlaySound
	ld a, [wEventFlags + EVENT_MOVEDEX_SORTING_MODE / 8]
	xor 1 << EVENT_MOVEDEX_SORTING_MODE % 8 ; toggle sorting mode
	ld [wEventFlags + EVENT_MOVEDEX_SORTING_MODE / 8], a
	call DrawMovedexSortPrompt
	xor a
	ld [wCurrentMenuItem], a
	ld [wListScrollOffset], a
...
.doneMarker
	...
	coord hl, 8, 6 ; location we will print power number
...
.donePrintingPower
	...
	call Divide
	ldh a, [H_REMAINDER] ; resultant accuracy percentage
	cp 123
	ldh a, [H_QUOTIENT + 3]
	jr c, .noRemainder
 	inc a
...
DrawBottomDataBorder: ; can change if there's no previous or next move
	...
	coord hl, 1, 17
	ld a, $CC ; start of <prev prompt
	ld [hl], a
	add 4
;	coord hl, 1, 16 ; now we do the top line of each's little box
;	ld [hli], a
;	inc a
;	ld [hli], a
;	inc a
...
.nextButton
	...
	cp b
	jr z, .noNext
	coord hl, 18, 17 ; now we do the next> prompt
	ld a, $CD ; start of <prev prompt
;	ld [hli], a
;	inc a
;	ld [hli], a
;	inc a
